// K-Shortest path O(VlogV + E + klogk)
struct Eppstein {
  #define x first
  #define y second
  using T = int;
  ...
  vector<int> h;
	Eppstein(vector<vector<Edge>>& G, int s, int t) {
		int n = G.size(); vector<vector<Edge>> H(n);
		for(int i = 0; i < n; i++) 
      for (Edge &e : G[i]) H[e.x].push_back({i, e.y});
		vector<int> ord, par(n, -1); vector<T> d(n, -INF);
		Q.push({d[t] = 0, t});
		while (!Q.empty()) {
			auto v = Q.top(); Q.pop();
			if (d[v.y] == v.x) {
				ord.push_back(v.y);
        for (Edge &e : H[v.y]) if (v.x-e.y > d[e.x]) {
